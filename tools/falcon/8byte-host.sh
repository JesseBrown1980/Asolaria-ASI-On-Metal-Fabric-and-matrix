...
RECV=$ROOT/8byte-receipts.ndjson; SUPV=$ROOT/8byte-supervisor.ndjson
...
while true; do
  ...
  grep -vxFf "$SEEN" "$TMP" 2>/dev/null | while IFS= read -r b; do
    ...
    echo "{\"ts\":\"$ts\",\"host\":\"$SER\",\"handle8\":\"$REAL\",\"msg\":\"$b\",\"state\":\"received_held_safe\"}" >> "$RECV"
    echo "{\"ts\":\"$ts\",\"host\":\"$SER\",\"msg\":\"$b\",\"stack\":{\"real\":\"$REAL\",\"self_reflect\":\"$REFL\",\"ask_fabric\":\"$FABR\"},\"fabric_pid0\":\"$PID0\",\"supervisor\":\"sees_all_three\",\"held_safe\":true}" >> "$SUPV"
