...
Any non-loopback caller must send one of:

```text
Authorization: Bearer <shared colony key>
X-Asolaria-Recall-Key: <shared colony key>
```

It must also send an owner PID accepted by this host:
...
peer is made server-side with the shared key.
